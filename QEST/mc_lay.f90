...
! *** g(theta=0) = 1
      ...
      IMPLICIT NONE
      DOUBLE PRECISION :: pi
      DOUBLE PRECISION :: g0_3d_karman, a, eps, k, nu
      pi=4.0*ATAN(1.0)
      g0_3d_karman = 2.0 * SQRT(pi) &
      & * DGAMMA(nu+1.5))) &
      & / DGAMMA(nu))) / (nu+0.5) * (k*eps)**2 * a &
      ...
      IMPLICIT NONE
      DOUBLE PRECISION :: pi
      DOUBLE PRECISION :: gtr_3d_karman, a, eps, k, nu
      pi=4.0*ATAN(1.0)
...
! *** computes energy in an acoustic scattering layer
! *** assuming anisotropic scattering and isotropic source radiation
